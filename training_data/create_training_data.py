...
#MCMC configuration
nwalkers, nsteps = 8, 400
nburn = int(np.sqrt(nsteps)) #500
...
from_scratch = False
if from_scratch:
    best_fit_models = np.zeros((N_boxes*N_z,2))
    np.savetxt("txt_files/best_fit_models.txt",best_fit_models)
    mean_models = np.zeros((N_boxes*N_z,2))
    np.savetxt("txt_files/mean_models.txt",mean_models)
    var_models = np.zeros((N_boxes*N_z,2))
...
for i in xrange(box_lo,box_hi):
    ...
    for j in xrange(z_lo,z_hi):
        ...
        if run_test:
            test = TL.lnprob(guesses,lM_bins,N_data,cov,volume,MF_model)
            print "Test result = %f\n"%test

        if run_best_fit:
            nll = lambda *args:-TL.lnprob(*args)
            result = op.minimize(nll,guesses,args=(lM_bins,N_data,cov,volume,MF_model),method="Powell")
        ...
        if run_mcmc:
            ...
            sampler = emcee.EnsembleSampler(nwalkers,ndim,TL.lnprob,args=(lM_bins,N_data,cov,volume,MF_model))
        ...
        if see_corner:
            ...
            fig = corner.corner(chain,labels=[r"$f$",r"$g$"],plot_datapoints=False)
...
#Save the models
np.savetxt("txt_files/best_fit_models.txt",best_fit_models,header="f\tg")
np.savetxt("txt_files/mean_models.txt",mean_models,header="f\tg")
np.savetxt("txt_files/var_models.txt",var_models,header="f\tg")
