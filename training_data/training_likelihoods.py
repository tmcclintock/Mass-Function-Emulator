...
def lnprior(params):
    ...
    if any(params[:4]<0) or any(params[:4]>10): return -np.inf
    if params[4] < -20 or params[4] > 5: return -np.inf
    return 0

#Likelihood
def lnlike(params,lM_bins,N_data,cov_data,volume,MF_model):
    d,e,f,g,lnscat2 = params
...
#Posterior
def lnprob(params,lM_bins,N_data,cov_data,volume,MF_model):
    f,g = params
    full_params = np.array([1.97,1.0,f,g,-19.0])
    #full_params = np.array([1.97,e,f,g,-19.0])
    lp = lnprior(full_params)
    if not np.isfinite(lp): return -np.inf
    return lp + lnlike(full_params,lM_bins,N_data,cov_data,volume,MF_model)
