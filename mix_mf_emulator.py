...
This version works with the rotated data, so
it builds emulators that emulate along the eigenvectors
and then rotates them back into the tinker parameters.
"""
import emulator, sys
import numpy as np
import tinker_mass_function as TMF

rpath = "../full_training_data/"
Ri = np.loadtxt(rpath+"Rint_matrix.txt").T
Rs = np.loadtxt(rpath+"Rslope_matrix.txt").T
R = [Ri,Rs]
...
class mf_emulator(object):
    ...
    def predict_parameters(self,cosmology):
        ...
        for i in range(0,2):
            means[i::2] = np.dot(means[i::2],R[i])
            errs[i::2] = np.dot(errs[i::2],R[i])
...
if __name__=="__main__":
    ...
    means = np.loadtxt("./full_training_data/mix_rotated_6params/mixed_dfg_means.txt")
    variances = np.loadtxt("./full_training_data/mix_rotated_6params/mixed_dfg_vars.txt")
